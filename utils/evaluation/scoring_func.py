...
def get_chem(mol):
    ...
    TPSA_score = Descriptors.TPSA(mol)
    HBA_score = Descriptors.NumHAcceptors(mol)
    HBD_score = Descriptors.NumHDonors(mol)
    Fsp3_score = Descriptors.FractionCSP3(mol)
    ROTB_score = Descriptors.NumRotatableBonds(mol)
    mol_weight = Descriptors.MolWt(mol)
    pains_score = is_pains(mol)
    return {
        'qed': qed_score,
        'sa': sa_score,
        'logp': logp_score,
