...
def center_pos(ligand_pos, batch_ligand):
...
class SeFMol_rigid(nn.Module):
    ...
    def forward(self, init_ligand_pos, init_ligand_v, batch_ligand, properties,
        ...
        outputs = self.refine_net(init_ligand_h, init_ligand_pos, mask_ligand, batch_ligand, return_all=return_all, fix_x=fix_x)
    ...
    def get_diffusion_loss(
            self, ligand_pos, ligand_v, batch_ligand, properties, time_step=None
    ):
        num_graphs = batch_ligand.max().item() + 1
        ligand_pos, _ = center_pos(
            ligand_pos, batch_ligand)

        # 1. sample noise levels
        if time_step is None:
            time_step, pt = self.sample_time(num_graphs, ligand_pos.device, self.sample_time_method)
        else:
            pt = torch.ones_like(time_step).float() / self.num_timesteps
