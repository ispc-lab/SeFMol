...
from models.sefmol_sfrl import SeFMol
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='./ckpt_test')
    parser.add_argument('--pretrained_ckpt_path', type=str, default='checkpoint/sefmol_rigid_pt.pt')
    ...
    ligand_featurizer = trans.FeaturizeLigandAtom(config.data.transform.ligand_atom_mode, ligand_path=config.data.path)
    transform_list = [
        protein_featurizer,
        ligand_featurizer,
    ...
    if args.pretrained_ckpt_path:
        pretrained_ckpt = torch.load(args.pretrained_ckpt_path, map_location=args.device)
        petrained_model = SeFMol_rigid(
            config.model,
            ligand_atom_feature_dim=ligand_featurizer.feature_dim
        ).to(args.device)
        petrained_model.load_state_dict(pretrained_ckpt['model'])
    else:
        petrained_model = None
    ...
    model = SeFMol(
        ...
        ligand_atom_feature_dim=ligand_featurizer.feature_dim,
        petrained_model=petrained_model
    ).to(args.device)
        ...
        for it in range(1, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                if best_loss is None or val_loss < best_loss:
                    ...
                    print(f'[Validate] Val loss is not improved. '
