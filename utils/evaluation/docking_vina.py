    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8, save_pose=False, **kwargs):  # seed=0 mean random seed
        ...
        if not save_pose: 
            ...
            if mode == 'score_only': 
                ...
                # with open('./tmp_dock/ligand_vina_out.pdbqt', 'w') as f: 
                v.write_poses('./tmp_dock/ligand_vina_out.pdbqt', n_poses=1, overwrite=True)
...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol_realworld(cls, ligand_rdmol, protein_path='./data/crossdocked', **kwargs):
        # load original pdb
        return cls(protein_path, ligand_rdmol, **kwargs)

    @classmethod
    def from_generated_mol_resgen(cls, ligand_rdmol, protein_path='./data/crossdocked', **kwargs):
        # load original pdb
        return cls(protein_path, ligand_rdmol, **kwargs)
      
    @classmethod
    def from_generated_mol_reward(cls, ligand_rdmol, protein_path, **kwargs):
        # load original pdb
        return cls(protein_path, ligand_rdmol, **kwargs)
