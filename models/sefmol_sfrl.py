...
def decoup_batch(inputs, inputs_batch):
    num_graphs = inputs_batch.max().item() + 1
    max_atom_nums = max(inputs[(inputs_batch == i)].shape[0] for i in range(num_graphs))
    outputs = []
    for i in range(num_graphs):
        mask = (inputs_batch == i)
        outputs.append(F.pad(inputs[mask], (0, 0, 0, max_atom_nums - inputs[mask].size(0))))
    return torch.stack(outputs)


def unpadding_func(padded_inputs, return_atom_nums=False):
    num_graphs = padded_inputs.size(0)
    outputs = []
    atom_nums = []
    for i in range(num_graphs):
        mask = (padded_inputs[i] != 0).any(dim=1)     
        unpadding_tensor = padded_inputs[i][mask]
        outputs.append(unpadding_tensor)
        atom_nums.append(unpadding_tensor.size(0))
    if return_atom_nums:
        return torch.cat(outputs), atom_nums
    else:
        return torch.cat(outputs)
    
def cal_log_ligand_v(log_model_prob, batch_ligand, batch_size, next_state_v=None):
    ligand_v_next_list, log_prob_v_list = [], []
    for i in range(batch_size):
        mask = (batch_ligand == i)
        if next_state_v != None:
            ligand_v_next = next_state_v[mask]
        else:
            ligand_v_next = log_sample_categorical(log_model_prob[mask])
        dist_v = Categorical(log_model_prob[mask])
        log_prob_v = (
            dist_v.log_prob(ligand_v_next.detach().clone()) # log_prob = π.log_prob(a)
            .mean(dim=-1)
        )
        ligand_v_next_list.append(ligand_v_next)
        log_prob_v_list.append(log_prob_v)

    return torch.cat(ligand_v_next_list, dim=0), torch.stack(log_prob_v_list)

def cal_log_ligand_pos(pos_model_mean, pos_log_variance, batch_ligand, ligand_pos, num_graphs, next_state_pos=None, nonzero_mask=None):
    log_prob_pos_list, ligand_pos_next_list = [], []

    for i in range(num_graphs):
        mask = (batch_ligand == i)
        std_dev_t = (0.5 * pos_log_variance[mask]).exp()
        dist_pos = Normal(pos_model_mean[mask], std_dev_t) 

        if next_state_pos != None:
            ligand_pos_next = next_state_pos[mask]
        else:
            ligand_pos_next = pos_model_mean[mask] + nonzero_mask[mask] * std_dev_t * torch.randn_like(  
                ligand_pos[mask])
        log_prob_pos = (
                dist_pos.log_prob(ligand_pos_next.detach().clone())    
                .mean(dim=-1)
                .mean(dim=-1)
            ) 

        log_prob_pos_list.append(log_prob_pos)
        ligand_pos_next_list.append(ligand_pos_next)
    return torch.cat(ligand_pos_next_list, dim=0), torch.stack(log_prob_pos_list)
...
def log_sub_exp(a, b):
    if a < b:
        raise ValueError("Result is undefined for log(exp(a) - exp(b)) when a < b.")
    maximum = torch.max(a, b)
...
def initialize_diffusion_params(model, config, num_timesteps):
    if config.beta_schedule == 'cosine':
            alphas = cosine_beta_schedule(num_timesteps, config.pos_beta_s) ** 2
    ...
    betas = to_torch_const(betas)
    alphas_cumprod = to_torch_const(alphas_cumprod)
    alphas_cumprod_prev = to_torch_const(alphas_cumprod_prev)
    ...
    posterior_mean_c0_coef = betas * np.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod)
    posterior_mean_ct_coef = (1. - alphas_cumprod_prev) * np.sqrt(alphas) / (1. - alphas_cumprod)
    # log calculation clipped because the posterior variance is 0 at the beginning of the diffusion chain
    posterior_logvar = to_torch_const(np.log(np.append(posterior_var[1], posterior_var[1:])))

    # atom type diffusion schedule in log space  
    if config.v_beta_schedule == 'cosine':
        alphas_v = cosine_beta_schedule(num_timesteps, config.v_beta_s)
        # print('cosine v alpha schedule applied!')
    else:
        raise NotImplementedError
    log_alphas_v = np.log(alphas_v)
    log_alphas_cumprod_v = np.cumsum(log_alphas_v)
    log_alphas_v = to_torch_const(log_alphas_v)
    log_one_minus_alphas_v = log_1_min_a(log_alphas_v)
    log_alphas_cumprod_v = to_torch_const(log_alphas_cumprod_v)
    log_one_minus_alphas_cumprod_v = log_1_min_a(log_alphas_cumprod_v)
    

    model['betas'] = betas
    model['Lt_history'] = torch.zeros(num_timesteps)
    model['Lt_count'] = torch.zeros(num_timesteps)
    model['alphas_cumprod'] = alphas_cumprod
    model['alphas_cumprod_prev'] = alphas_cumprod_prev
    model['sqrt_alphas_cumprod'] = sqrt_alphas_cumprod
    model['sqrt_one_minus_alphas_cumprod'] = sqrt_one_minus_alphas_cumprod
    model['sqrt_recip_alphas_cumprod'] = sqrt_recip_alphas_cumprod
    model['sqrt_recipm1_alphas_cumprod'] = sqrt_recipm1_alphas_cumprod
    model['posterior_mean_c0_coef'] = posterior_mean_c0_coef
    model['posterior_mean_ct_coef'] = posterior_mean_ct_coef.float()
    model['posterior_var'] = posterior_var
    model['posterior_logvar'] = posterior_logvar
    model['log_alphas_v'] = log_alphas_v
    model['log_alphas_cumprod_v'] = log_alphas_cumprod_v
    model['log_alphas_v'] = log_alphas_v
    model['log_one_minus_alphas_v'] = log_one_minus_alphas_v
    model['log_one_minus_alphas_cumprod_v'] = log_one_minus_alphas_cumprod_v

def merge_node_property(ligand_v, p_feat, batch_size, batch_ligand):
    p_feat = p_feat.reshape(batch_size, -1)
    new_feature_dim = ligand_v.shape[-1] + p_feat.shape[-1] 
    new_init_ligand_v = torch.zeros(ligand_v.shape[0], new_feature_dim).to(ligand_v.device)
    for i in range(batch_size):
        mask = (batch_ligand == i)
        ligand_masked = ligand_v[mask] 
        p_feat_masked = p_feat[i].repeat(ligand_masked.shape[0], 1) 
        new_init_ligand_v[mask] = torch.cat([ligand_masked, p_feat_masked], dim=-1)
    return new_init_ligand_v

# Model
class SeFMol(nn.Module):
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, n_timesteps=1000, petrained_model=None):
        super().__init__()
        self.config = config
        ...
        if config.beta_schedule == 'cosine':
            alphas = cosine_beta_schedule(self.num_timesteps, config.pos_beta_s) ** 2
            betas = 1. - alphas
        else:
            betas = get_beta_schedule(
                beta_schedule=config.beta_schedule,
        ...
        if petrained_model:
            self.ligand_atom_emb = petrained_model.ligand_atom_emb
        else:
            if self.time_emb_dim > 0:
                if self.time_emb_mode == 'simple':
                    ...
                    self.ligand_atom_emb = nn.Linear(ligand_atom_feature_dim + self.time_emb_dim + self.config.condition_hidden_dim, emb_dim)
                else:
                    raise NotImplementedError
            else:
                self.ligand_atom_emb = nn.Linear(ligand_atom_feature_dim + self.config.condition_hidden_dim, emb_dim)

        if petrained_model:
            self.refine_net = petrained_model.refine_net
        else:
        ...
        self.condition_embedding = nn.Sequential(
            nn.Linear(self.config.condition_dim, self.hidden_dim),
            ShiftedSoftplus(),
            nn.Linear(self.hidden_dim, self.config.condition_hidden_dim),
        )

    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand, properties,
        ...
        if self.config.condition_dim:
            property_feature = self.condition_embedding(properties.reshape(batch_size, -1))
            init_ligand_v = merge_node_property(init_ligand_v, property_feature, batch_size, batch_ligand)
    ...
    def q_v_pred(self, log_v0, t, batch):
        # compute q(vt | v0)
        log_cumprod_alpha_t = extract(self.log_alphas_cumprod_v, t, batch)      # alpha
        log_1_min_cumprod_alpha = extract(self.log_one_minus_alphas_cumprod_v, t, batch)  # beta

        log_probs = log_add_exp(        # z = x_t-1 * alpha_t + beta_t / K 
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, properties, time_step=None
        ):
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand=None, properties=None, time_steps=None, state=None, next_state=None,
                         num_steps=None, center_pos_mode=None, log_prob_mode='step_logprob'):
        if log_prob_mode == 'step_logprob':
            ...
            pos_traj, v_traj, log_prob_traj = [], [], []
            
            # init traj_list
            pos_traj.append(init_ligand_pos)
            v_traj.append(init_ligand_v)
            ...
            for i in time_seq:
                ...
                log_prob_traj.append(log_prob_pos)    
        ...
        elif log_prob_mode == 'step_forward_logprob':            
            batch_size = state.size(0)
 
            # state pos_v     
            state_pos = unpadding_func(state)[:, :3]
            state_pos = state_pos.to(protein_pos)
            state_v = unpadding_func(state)[:, 3:].squeeze(1).long().to(protein_pos.device)
         
            # next_state pos_v
            next_state_pos = unpadding_func(next_state)[:, :3].to(protein_pos.device)
            next_state_v = unpadding_func(next_state)[:, 3:].squeeze(1).long().to(protein_pos.device)
            next_state_v = F.one_hot(next_state_v, self.num_classes).float()

            t = time_steps.long().to(protein_pos.device)
     
            preds = self(
                protein_pos=protein_pos,
                protein_v=protein_v,
                batch_protein=batch_protein,
                init_ligand_pos=state_pos,
                init_ligand_v=state_v,
                batch_ligand=batch_ligand,
                properties=properties,
                time_step=t
            )
            # Cal noise
            pred_pos_noise = preds['pred_ligand_pos'] - state_pos
     
            # Compute posterior mean and variance
            if self.model_mean_type == 'noise':
                pos0_from_e = self._predict_x0_from_eps(xt=state_pos, eps=pred_pos_noise, t=t, batch=batch_ligand)
                v0_from_e = preds['pred_ligand_v']
            elif self.model_mean_type == 'C0':
                pos0_from_e = preds['pred_ligand_pos']
                v0_from_e = preds['pred_ligand_v']
            ...
            # cal pos and pos_log_prob 
            pos_log_variance = extract(self.posterior_logvar, t, batch_ligand)
            pos_model_mean = self.q_pos_posterior(x0=pos0_from_e, xt=state_pos, t=t, batch=batch_ligand)     
            _, log_prob_pos = cal_log_ligand_pos(pos_model_mean, pos_log_variance, batch_ligand, state_pos, batch_size, next_state_pos=next_state_pos)
 
            log_prob = log_prob_pos
        ...
        elif log_prob_mode == 'rigid_sample':
            if num_steps is None:
                num_steps = self.num_timesteps
            num_graphs = batch_protein.max().item() + 1
